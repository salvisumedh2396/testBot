...
class Deadline(commands.Cog):
    ...
    async def deleteReminder(self, ctx, *, hwName):
        author = ctx.message.author
        to_remove = []
        for reminder in self.reminders:
            #print('in json '+str(reminder["HOMEWORK"])+' hwName '+hwName)
            if reminder["HOMEWORK"] == hwName:
                #print('true '+hwName)
    ...
    async def change_due_date(self, ctx,classid: str, hwid: str, *, date: str):
        ...
        for reminder in self.reminders:
            ...
            if ((reminder["HOMEWORK"] == hwid) and (reminder["COURSE"] == classid)):
                ...
                if(flag):
                    json.dump(self.reminders, open("data/remindme/reminders.json", "w"))
                    await ctx.send("{} {} has been updated with following date: {} by {}".format(classid, hwid, reminder["DUEDATE"], reminder["ID"]))
    ...
    async def remindme(self, ctx, quantity: int, time_unit : str,*, text :str):
        ...
        if not time_unit in self.units:
            await ctx.send("Invalid unit of time. Select from minutes/hours/days/weeks/months")
            return
        if quantity < 1:
    ...
    async def listreminders(self, ctx):
        ...
        for reminder in self.reminders:
            ...
            try:
                #await ctx.send("{} homework named: {} which is due on: {} by {}".format(self.client.get_user(reminder["ID"]), reminder["TEXT"]))
                await ctx.send("{} homework named: {} which is due on: {} by {}".format(reminder["COURSE"],reminder["HOMEWORK"],reminder["DUEDATE"],self.client.get_user(reminder["ID"])))
    ...
    async def delete_old_reminders(self):
        while self is self.client.get_cog("Deadline"):
            to_remove = []
            for reminder in self.reminders:
                if reminder["FUTURE"] <= int(time.time()):
                    try:
